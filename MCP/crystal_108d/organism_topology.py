...
def classify_entity(
    ...
    if vertex_mask == 0:
        # Check if scores are truly undifferentiated (all equal or near-zero)
        score_vals = list(scores.values())
        score_range = max(score_vals) - min(score_vals)
        if score_range < 0.05:
            # Undifferentiated: classify as SFCR (aether) not default-S
            vertex_mask = 15  # SFCR
        else:
            # Some differentiation exists but below threshold: use dominant
